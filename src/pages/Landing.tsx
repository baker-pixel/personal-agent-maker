        ...
        <div className="container max-w-4xl text-center px-5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }}>
            <Button onClick={goOnboard} size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90 text-sm md:text-base px-6 md:px-8 py-5 md:py-6 rounded-xl shadow-lg shadow-accent/20 w-full sm:w-auto">
              Meet your Normy <ArrowRight className="w-5 h-5 ml-2" />
            </Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-14 md:py-20">
        <div className="container max-w-5xl px-5">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="text-center mb-10 md:mb-14">
            ...
            <motion.p variants={fadeUp} custom={2} className="text-muted-foreground text-base md:text-lg max-w-2xl mx-auto">Most AI is built by techies, for techies — and it talks like a robot. Normy is the opposite on both counts: built for normies, and powered by the world's first personality-driven AI.</motion.p>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
            <motion.div variants={fadeUp} custom={3} className="rounded-2xl border-2 border-accent/20 bg-background p-6 md:p-8 shadow-md">
              ...
              <p className="text-muted-foreground text-sm md:text-base leading-relaxed">We trained your agent on a database of 32M personality profiles, to truly understand <span className="font-semibold text-foreground">you</span>, your personality, your communication preferences, work style, and 29 other aspects of how you like to operate — so it just "gets you." The result is an assistant you actually enjoy speaking to and can genuinely trust with your inbox, calendar, and relationships.</p>
            </motion.div>
